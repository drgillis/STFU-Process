...
Simply enable STFU Process mode in a comint process buffer (`M-x
stfu-process-mode`). All outputs returned longer than the defined maximum
output length or individual line length will be truncated and replaced with a
customizable replacement string (`stfu-process-suppression-string` and
`stfu-process-suppression-long-line-string`, respectively).
...
interrupt the stream of the process when possible (typically if it is
...
The default limits are 100,000 characters for total output
(`stfu-process-total-limit`) and 5,000 characters per line
(`stfu-process-line-limit`). Customize these according to your pain tolerance.
...
naming/ /collision in the extraordinarily likely scenario where someone else
